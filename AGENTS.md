...
This section is the authoritative technical handoff for INCONNECT. It supplements the repository-wide instructions above and describes current capabilities rather than implementation chronology. The 2026-09-24 checkpoint on `feature/inconnect-messaging` includes Twilio inbound, secure inbound media ingestion into `FileEntity`/`FileStorage`, authorized attachment access, rich inbound `IMAGE`, `STICKER`, `AUDIO`, `VIDEO`, `DOCUMENT`, `CONTACT`, and `LOCATION` content, media-ready realtime updates, real frontend media rendering, the authorized Read API, the native inbox, outbound free-form Messaging, server-authoritative WhatsApp session policy, durable dispatch, templates, the functional composer and template picker, secure outbound media backend preparation, actor/workspace-scoped upload staging, deterministic and idempotent server-owned file identities, `FileStorage`/`FileEntity` outbound preparation, transactional outbound `Message` + `Attachment` consumption, provider-neutral media dispatch, Twilio outbound media delivery, an expiring provider-media capability, metadata GraphQL upload mutations, the user-facing attachment composer, the secure dynamic CRM Conversation context backend, the visible dynamic CRM Conversation context panel, the native CRM context configuration UI in Settings, secure manual CRM candidate search, and atomic manual `UNASSIGNED -> LINKED` backend linking. Manual linking is dynamic-anchor-driven, TRIAGE-authorized, protected by PostgreSQL Conversation and target locks, and persists a durable `CONVERSATION_LINKED` Outbox transition. CRM context is implemented end to end: ordered `ContextField` presentation configuration, an authorized CRM record context query, standard object and field permission enforcement, INCONNECT Record Access on the final CRM `SELECT`, safe normalized field DTOs, management APIs, live authorized context reads for the selected Conversation, a desktop third-pane experience, a narrow/mobile fullscreen context experience, and ordered field administration at Settings -> Messaging -> CRM context. The runtime panel remains read-only, and Settings configures presentation fields only; neither surface adds anchor selection/change, CRM field editing, matching, related lists, or CRM realtime. Fase 12A implements the manual-linking backend only: the visible Fase 12B search, candidate-selection, confirmation, and Link UI are not implemented. The composer provides a file picker, secure outbound upload UX, honest selection/upload/finalization states, retry and remove, local previews, capability-driven captions, and media send through opaque `outboundUploadIds`. The native inbox also provides server-backed `ALL | UNREAD | FAVORITES | PENDING` views, unread visual treatment, personal Favorite controls, shared Pending controls, manual Mark Read/Mark Unread, and conservative automatic read through a server-derived snapshot target.
...
Fase 12A implements the backend for explicit human linking. It does not implement visible linking UI. The current product flow is:
...
Once linked, existing runtime authorization and context rules apply. Fase 12A adds no frontend context-refresh lifecycle. The current Fase 10B panel remains read-only and has no visible search, candidate list, Link button, confirmation, or Unlink button; Fase 12B visible linking UI remains pending.
...
For `LINKED`, the panel renders the safe object label, canonical `recordLabel` when available, and configured fields in server-returned order. The raw `recordId` is not the primary visible identity. A null `recordLabel` displays the localized `Unlabeled record`; it never falls back to the record ID, first configured Field, phone, email, or object label. For `UNASSIGNED`, the existing panel displays the localized `No CRM record linked.` and the panel itself performs no search, create, matching, auto-link, linking action, or additional CRM lookup. The separate Fase 12A candidate-search query and link mutation are backend-only and are not invoked by this panel. `LINKED` with an empty `fields` list is valid: the object/record summary remains and the panel displays `No context fields configured.` without fetching arbitrary fields or inferring defaults.
...
Manual smoke was not executed because no already-authenticated local frontend session was available and a Settings Save would have mutated persistent local configuration without explicit authorization. No data, migration, or configuration was created merely to force smoke testing; this remains an explicit QA follow-up, not a claimed manual-smoke success. Local validation does not imply production deployment.
...
- There is no auto-link, matching, phone/email matching, confidence score, sender-derived CRM lookup, or automatic record creation. Visible linking UI remains a separate pending frontend phase.
...
- Treat `UNASSIGNED` as a neutral runtime-panel display state. The current panel must never trigger matching, linking, creation, or another CRM lookup; the backend-only Fase 12A APIs do not make the panel a linking UI. Zero configured fields is valid and must never cause arbitrary fallback-field selection.
